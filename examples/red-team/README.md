...
`run_blast_nodes_demo`, `run_bank_color_demo`, `run_qos_credits_demo`,
`run_outside_slice_demo`, `run_silent_remote_demo`, `run_firewall_demo`,
`run_softsfi_demo`, `run_softnoi_demo`, and `run_sva_demo`. Not a new isolator,
not a QEMU guest, not a slide. Blast-nodes needle: `[redteam] attack=blast-nodes result=refused`
(`admit_nodes` → `BlastRadius`; hops stays `attack=blast-hops`). Bank-color needle:
