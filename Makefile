...
red-team:
	...
	grep -q "\\[redteam\\] attack=blast-nodes result=refused" $(REDTEAM_LOG)
	grep -q "\\[redteam\\] attack=bank-color result=refused" $(REDTEAM_LOG)
	...
	grep -q "\\[redteam\\] attack=smmu-not-mapped result=refused" $(REDTEAM_LOG)
	grep -q "\\[redteam\\] attack=smmu-wrong-stream result=refused" $(REDTEAM_LOG)
	grep -q "\\[redteam\\] attack=smmu-stream-abort result=refused" $(REDTEAM_LOG)
	...
	grep -F -x -q "[softsfi] unknown-base=refused" $(REDTEAM_LOG)
...
qemu-ci: $(LOADER_ELF)
	...
	if { [ $$ec -eq 0 ] || [ $$ec -eq 1 ]; } \
	   ...
	   && grep -q "\\[sid\\] SET_SID-at-submit two tenants" $(BUILD)/qemu-serial.log \
	   && grep -q "\\[sid\\] Soft SMMU refuse until submit SID" $(BUILD)/qemu-serial.log \
	   ...
	   && grep -q "\\[softcct\\] package fences=" $(BUILD)/qemu-serial.log \
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/qemu-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/qemu-serial.log \
...
qemu-pcid-ci: $(LOADER_ELF)
	...
	if { [ $$ec -eq 0 ] || [ $$ec -eq 1 ]; } \
	   ...
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/qemu-pcid-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/qemu-pcid-serial.log \
...
qemu-nopcid-ci: $(LOADER_ELF)
	...
	if { [ $$ec -eq 0 ] || [ $$ec -eq 1 ]; } \
	   ...
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/qemu-nopcid-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/qemu-nopcid-serial.log \
...
qemu-smp-ci: $(LOADER_ELF)
	...
	if grep -q "\\[smp\\] AP 1 online" $(BUILD)/smp-serial.log \
	   ...
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/smp-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/smp-serial.log \
...
qemu-blk-ci: $(LOADER_ELF) $(BOOTFS_IMG)
	...
	if { [ $$ec -eq 0 ] || [ $$ec -eq 1 ]; } \
	   ...
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/qemu-blk-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/qemu-blk-serial.log \
...
qemu-riscv-ci: $(RV_ELF)
	...
	if grep -q "FABRIC IPC + TENSOR ARENA + ACCEL JOB COMPLETE" $(BUILD)/riscv-serial.log \
	   ...
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/riscv-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/riscv-serial.log \
...
qemu-aarch64-ci: $(AA_ELF)
	...
	if grep -q "FABRIC IPC + TENSOR ARENA + ACCEL JOB COMPLETE" $(BUILD)/aarch64-serial.log \
	   ...
	   && grep -q "\\[softcct\\] two-chiplet producer/consumer elision sealed" $(BUILD)/aarch64-serial.log \
	   ...
	   && grep -q "\\[softsfi\\] unknown-base=refused" $(BUILD)/aarch64-serial.log \
