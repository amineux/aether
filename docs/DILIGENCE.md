...
| `cargo test -p aether-pjrt` | PJRT Add/Relu/Mul/Max (`function` 2 / 3 / 4 / 5) as research ops on the frozen packet |
...
| `[scope] soft≠strict` | Honesty: Soft remote-steal ≠ Strict refuse on the same `ChipletTaskScope` job (`run_chiplet_scope_demo()`). Thin exploration stub; not ChipletFleet calendar, not SpectralCut elevate. Boot demo may still leave `chiplet_scope: None` (unscoped). |
...
| PJRT/IREE-shaped host nouns | Types + working host session; Event create/record/wait on existing SoftChipletSync fences (PR #74); Add / Relu / Mul / Max on the frozen packet (PR #84 / #88 / #92); mp-shim exercises Mul/Max too; no graph IR | `core/src/abi.rs`, `host/aether-pjrt`, `host/aether-mp-shim`, `docs/{ABI,HOST}.md` |
...
new IR. Not partner latency. After SyncScope collapse (#102
...
`run_chiplet_scope_demo()` is the Soft≠Strict clip (host `[scope] soft≠strict`):
Strict refuses remote steal of a scoped job; Soft allows it after local-first.
Thin stub only — not ChipletFleet calendar. Boot `run_boot_demo` may still use
`chiplet_scope: None` (unscoped jobs are unrestricted under both policies).
...
caller of that image (research-sketch doorbell). `host/aether-mp-shim`
is a MicroPerceptron-shaped thin consumer of the same image
(inspiration name only; secondary to PJRT; not a port).
...
| Red-team clip | `make red-team` | Host stdout; greps `[redteam] attack=… result=refused` plus fabric-class / `ATOMIC_ADD` / `[softsfi] tensor=refused` / `[softsfi] heap=refused` / `[softsfi] unknown=refused` / `[softsfi] unknown-base=refused` / `typed-window-sid` / `hbm-bw` / `xqueue-sid-override` / `set-sid-unbound` / `submit-sid` / `sid-budget` / `stage2-fault` / `softnoi-exhausted` / `hodge-harmonic-tree` / `hodge-curl-tree` / `hodge-quota` / `firewall-ident-pa` / `greenctx-overcommit` / `greenctx-unbound` / `greenctx-exhausted` / `greenctx-busy` / `smmu-overlap` / `smmu-not-mapped` / `smmu-wrong-stream` / `smmu-stream-abort` / `softcct-incorrect-elision` / `softcct-credit-exhausted` / `foreign-tenant-color` and the “what this is not” closer |
...
| Per-tenant SID pool exhausted | `run_sid_budget_demo` — Soft-SMMU `bind_stream` → `MapError::SidBudget` when over `SID_BUDGET_PER_TENANT` (peer still has budget). **Not** set-sid-unbound / SubmitSid / xqueue Busy; software pool only | refused |
| Nested Stage-2 dropped | `run_stage2_fault_demo` — Soft-SMMU `unbind_stage2` then walk → `MapError::Stage2Fault` (S1 remains; SID still Bound). **Not** PASID stale / SubmitSid / StreamAbort | refused |
| SoftNoI tenant slots full | `run_softnoi_exhausted_demo` — SoftNoI `admit` → `NoiError::Exhausted` past `MAX_NOI_TENANTS` (two light OK). **Not** softnoi-is OverBudget / fabric-class RingExhausted | refused |
...
| Empty HodgeQuota admit | `run_hodge_quota_demo` — `HodgeQuota::empty().admit` → `HodgeError::QuotaExceeded` (generous admits). **Not** HarmonicTreeReduce / CurlOnTree / ClassNotAuthorized / CapTable | refused |
...
| Soft-SMMU unbound walk abort | `run_smmu_stream_abort_demo` — Soft-SMMU `walk` / `translate_result` / `resolve_result` → `MapError::StreamAbort` on Unbound SID (bound+pin admits; after `unbind_stream` aborts again). **Not** set-sid-unbound Soft-CP Fault / SubmitSid / NotMapped / WrongStream | refused |
