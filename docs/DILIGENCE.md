...
| No PIE-KASLR / `fork` COW | HH + boot-time slide + KPTI + PCID + one-page COW subset landed (`ffffffff80000000+PA` + 16 MiB slots; user CR3 has no HH / no identity DMA; tagged `mov cr3` when CPUID.PCID, else full flush; `USER_COW_BASE` RO until write). Identity 4 GiB stays on kernel CR3 for DMA. Unused HH alias stays (not PIE). Not Meltdown-complete, not POSIX `mmap` |
